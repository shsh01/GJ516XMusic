...
from GJ516X import YouTube, app
from GJ516X.core.call import GJ516
from GJ516X.misc import SUDOERS, db
from GJ516X.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from GJ516X.utils.decorators.language import languageCB
from GJ516X.utils.formatters import seconds_to_min
from GJ516X.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from GJ516X.utils.stream.autoclear import auto_clean
from GJ516X.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await GJ516.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await GJ516.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await GJ516.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await GJ516.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await GJ516.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
